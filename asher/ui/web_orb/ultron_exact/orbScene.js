...
export function createOrbScene(container             )              {
  ...
  const chromaticShader = {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // ASHER electric-blue remap. Same ULTRON shader behavior;
        // only the RGB identity changes.
        gl_FragColor.rgb = mix(gl_FragColor.rgb, gl_FragColor.rgb * vec3(0.60, 0.78, 1.22), 0.3);
      }
    `,
  };
  ...
  // ——— COLORS ———
  const C_BRIGHT = 0x4f9dff;
  const C_MID = 0x2367d1;
  const C_DIM = 0x123f8a;
  const C_FAINT = 0x081d3d;
  const C_HOT = 0xaed5ff;
  ...
  function makeTextSprite(text        , size = 0.08) {
    ...
    ctx.fillStyle = `rgba(${(60 + Math.random() * 50) | 0}, ${(120 + Math.random() * 50) | 0}, 255, ${alpha})`;
  ...
  g.addColorStop(0.2, "rgba(45,110,235,0.6)");
  g.addColorStop(0.5, "rgba(20,65,160,0.15)");
  g.addColorStop(1, "rgba(5,25,70,0)");
  dCtx.fillStyle = g;
